    ...
    return `
      <div class="week-item ${colorClass}">
        ...
        <span class="week-title">
          <a href="week.html?course=${courseKey}&week=${w.week}">${w.title}</a>
        </span>
        <span class="week-icons">
          <span class="icon-badge ${hasSlides ? 'active' : ''}">スライド</span>
          <span class="icon-badge ${hasProject ? 'active project' : ''}">配布</span>
          <span class="icon-badge ${hasAssignment ? 'active assignment' : ''}">課題</span>
        </span>
      </div>
    `;
  }).join("");
